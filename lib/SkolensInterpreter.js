...
export default class SkolensInterpreter extends SkolensVisitor {
  ...
  visitWhileLoop(ctx) {
    ...
    while (this.#isValueTruthy(this.visit(ctx.getChild(2)))) {
      ...
      for (const entry of this.#getCurrentScope().entries()) {
        if (!varsBeforeLoop.includes(entry[0])) {
          this.#removeVarFromCurrentScope(entry[0]);
        }
      }
    }
  ...
  visitForLoop(ctx) {
    ...
    this.#removeVarFromCurrentScope(varName);
  }
  ...
  visitNewAssign(ctx) {
    ...
    if (!this.#typeMatchesValue(varType, varVal)) {
      this.#throwError(ctx.getChild(0).start.line, `nevar glabāt '${typeof varVal}' tipa vērtību mainīgajā ar tipu '${varType}'`);
    }
  ...
  visitCompOp(ctx) {
    ...
    if (typeof leftVal != 'number' || typeof rightVal != 'number') {
      this.#throwError(ctx.getChild(1).start.line, `salīdzinājuma operāciju ${op} var izmantot tikai lai salīdzinātu skaitļus`);
    }
  ...
  visitEqualityOp(ctx) {
    ...
    if (typeof leftVal != typeof rightVal) {
      this.#throwError(ctx.getChild(1).start.line, `${op.symbol.type == SkolensParser.NOTEQ ? 'ne' : ''}vienādības operāciju ${op} var izmantot tikai ar vienāda tipa vērtībām`);
    }
  ...
  visitConcatOp(ctx) {
    ...
    if (typeof leftVal == 'boolean') leftVal = leftVal ? 'patiess' : 'nepatiess';
    if (typeof rightVal == 'boolean') rightVal = rightVal ? 'patiess' : 'nepatiess';
